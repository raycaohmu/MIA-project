...
def compute_interesting_patches(
    ...
    pre_generated_thumbnails: dict = None
    ...
    """
    logger.info("About to compute interesting patches...")
    import time
    ...
    # 计算实际的下采样因子
    actual_downsample_factor = int(2 ** (tiles.level_count - largest_single_level - 1))
    logger.debug(f"计算得到的下采样因子: {actual_downsample_factor}")
    
    # 添加调试信息
    if pre_generated_thumbnails:
        logger.debug(f"预生成缩略图中的键: {list(pre_generated_thumbnails.keys())}")
    
    # 尝试使用所有可能的键格式
    thumb_found = False
    if pre_generated_thumbnails:
        # 尝试精确匹配
        exact_key = f"downsample_{actual_downsample_factor}"
        if exact_key in pre_generated_thumbnails:
            logger.info(f"使用精确匹配的预生成缩略图: {exact_key}")
            thumb = pre_generated_thumbnails[exact_key]
            # 确保缩略图是RGB格式（3通道）
            if thumb.mode == 'RGBA':
                logger.info(f"将RGBA格式缩略图转换为RGB格式")
                thumb = thumb.convert('RGB')
            full_tile = np.array(thumb, dtype=np.uint8)
            thumb_found = True
        
        # 如果没有精确匹配，尝试使用最接近的缩略图
        elif "thumbnail" in pre_generated_thumbnails:
            logger.info("使用通用缩略图")
            thumb = pre_generated_thumbnails["thumbnail"]
            # 确保缩略图是RGB格式（3通道）
            if thumb.mode == 'RGBA':
                logger.info(f"将RGBA格式缩略图转换为RGB格式")
                thumb = thumb.convert('RGB')
            # 获取原始尺寸
            orig_width, orig_height = tiles.level_dimensions[0]
            # 获取目标级别尺寸
            target_width, target_height = tiles.level_dimensions[largest_single_level]
            
            # 调整缩略图大小以匹配目标级别
            resized_thumb = thumb.resize((target_width, target_height), Image.Resampling.LANCZOS)
            full_tile = np.array(resized_thumb, dtype=np.uint8)
            thumb_found = True
    
    # 如果没有找到适合的缩略图，回退到原始方法
    if not thumb_found:
        logger.info(f"未找到合适的预生成缩略图，使用原始方法获取缩略图")
        thumb = slide.get_thumbnail(tiles.level_dimensions[largest_single_level])
        # 确保缩略图是RGB格式（3通道）
        if thumb.mode == 'RGBA':
            logger.info(f"将OpenSlide生成的RGBA格式缩略图转换为RGB格式")
            thumb = thumb.convert('RGB')
        full_tile = np.array(thumb, dtype=np.uint8)

    if len(full_tile.shape) == 3 and full_tile.shape[2] == 4:
        logger.info(f"检测到RGBA数组 (形状: {full_tile.shape})，转换为RGB")
        full_tile = full_tile[:, :, :3]
    diff_height = abs(
    ...
    logger.info(f"compute interesting completed in {time.time() - start_time:.2f} seconds")

    if tissue_annotation is None or len(tissue_annotation) == 0:
        # 在传递给generate_tissue_mask前再次检查full_tile的通道数
        if len(full_tile.shape) == 3 and full_tile.shape[2] == 4:
            logger.info(f"在传递给generate_tissue_mask前再次检查并转换RGBA (形状: {full_tile.shape})")
            full_tile = full_tile[:, :, :3]
    ...
    # 确保full_tile是RGB格式（3通道）用于创建图像
    if len(full_tile.shape) == 3 and full_tile.shape[2] == 4:
        logger.info(f"在创建可视化图像前再次转换RGBA为RGB (形状: {full_tile.shape})")
        full_tile = full_tile[:, :, :3]
    ...
    if len(polygons) != 0:
        # 确保full_tile是RGB格式（3通道）用于生成多边形概览
        if len(full_tile.shape) == 3 and full_tile.shape[2] == 4:
            logger.info(f"在生成多边形概览前再次转换RGBA为RGB (形状: {full_tile.shape})")
            full_tile = full_tile[:, :, :3]
...
def generate_thumbnails(
    slide: OpenSlide,
    slide_mpp: float,
    sample_factors: List[int] = [128],
    mpp_factors: List[float] = []
) -> dict:
    """使用FAST库启发的方法生成缩略图，专注于区块读取和流处理"""
    import numpy as np
    ...
    # 获取SVS文件路径
    slide_path = getattr(slide, '_filename', None)
    
    # 优化2: 尝试从预定义的缩略图目录加载
    thumb_dir = os.path.join(os.path.dirname(slide_path), 'thumbnails')
    slide_name = os.path.basename(slide_path)
    slide_id = os.path.splitext(slide_name)[0]
    cached_thumb_path = os.path.join(thumb_dir, f"{slide_id}_clean_thumb.jpg")  # 使用新命名避免加载已有网格的缩略图
    
    if os.path.exists(cached_thumb_path):
        try:
            thumbnail = Image.open(cached_thumb_path)
            for factor in sample_factors:
                thumbnails[f"downsample_{factor}"] = thumbnail
            thumbnails["thumbnail"] = thumbnail
            logger.debug(f"使用预缓存缩略图: {cached_thumb_path}")
            return thumbnails
        except Exception as e:
            logger.warning(f"加载预缓存缩略图失败: {e}")
    ...
    # 如果没有找到合适的级别，使用最低级别
    if suitable_level is None:
        suitable_level = level_count - 1
    
    level_dims = slide.level_dimensions[suitable_level]
    width, height = level_dims
    
    # 如果选定级别仍然太大，使用区块读取，但不使用PIL.paste
    if width > 10000 or height > 10000:
        # 计算最终缩略图大小
        sample_factor = sample_factors[0]
        orig_width = int(slide.properties["openslide.level[0].width"])
        orig_height = int(slide.properties["openslide.level[0].height"])
        target_width = int(orig_width / sample_factor)
        target_height = int(orig_height / sample_factor)
        
        # 计算缩放比例
        x_ratio = target_width / width
        y_ratio = target_height / height
        
        # 【修复】调整区块逻辑，确保区块边界完美匹配
        block_size = 1024
        
        # 计算区块放置位置的调整函数
        def calculate_target_position(start_pos, ratio):
            """精确计算区块在目标图像中的位置"""
            return int(start_pos * ratio + 0.5)  # 添加0.5并取整以获得更精确的四舍五入
        
        # 创建一个numpy数组作为目标缩略图
        thumbnail_array = np.zeros((target_height, target_width, 3), dtype=np.uint8)
        
        def process_block(block_info):
            """处理单个图像块"""
            x_start, y_start = block_info
            ...
            # 读取块
            try:
                block = slide.read_region(
                    (x_start * (2 ** suitable_level), y_start * (2 ** suitable_level)),
                    suitable_level,
                    (x_size, y_size)
                ).convert('RGB')
                
                # 精确计算目标位置和大小
                target_x = calculate_target_position(x_start, x_ratio)
                target_y = calculate_target_position(y_start, y_ratio)
                
                # 计算目标块的精确大小
                next_x = calculate_target_position(x_start + x_size, x_ratio)
                next_y = calculate_target_position(y_start + y_size, y_ratio)
                
                target_width_block = max(1, next_x - target_x)
                target_height_block = max(1, next_y - target_y)
                
                # 调整块大小 - 使用精确计算的目标尺寸
                resized_block = block.resize((target_width_block, target_height_block), Image.Resampling.LANCZOS)
                
                # 转换为numpy数组
                block_array = np.array(resized_block)
                
                return (target_x, target_y, block_array)
            except Exception as e:
                logger.warning(f"处理区块失败: {e}")
                return None
        
        # 创建块信息列表 - 确保精确覆盖整个图像
        x_positions = list(range(0, width, block_size))
        y_positions = list(range(0, height, block_size))
        blocks = [(x, y) for y in y_positions for x in x_positions]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            ...
            for result in results:
                ...
                target_x, target_y, block_array = result
                
                # 确保不超出边界
                h, w = block_array.shape[:2]
                h = min(h, thumbnail_array.shape[0] - target_y)
                w = min(w, thumbnail_array.shape[1] - target_x)
                
                if h > 0 and w > 0:
                    # 直接将区块放入numpy数组，不使用PIL的paste
                    thumbnail_array[target_y:target_y+h, target_x:target_x+w] = block_array[:h, :w]
        ...
        # 对于较小的图像，可以直接读取整个级别
        level_img = slide.read_region((0, 0), suitable_level, level_dims).convert('RGB')
        ...
        # 只需一次调整大小
        sample_factor = sample_factors[0]  # 使用第一个采样因子
        target_width = int(orig_width / sample_factor)
        target_height = int(orig_height / sample_factor)
        
        # 调整大小生成缩略图
        thumbnail = level_img.resize((target_width, target_height), Image.Resampling.LANCZOS)
    ...
    try:
        ...
        # 删除可能存在的旧版本
        old_thumb_path = os.path.join(thumb_dir, f"{slide_id}_thumb.jpg")
        if os.path.exists(old_thumb_path):
            try:
                os.remove(old_thumb_path)
                logger.debug(f"删除旧缓存: {old_thumb_path}")
            except:
                pass
                
        thumbnail.save(cached_thumb_path, quality=95)
        logger.debug(f"保存缩略图到: {cached_thumb_path}")
    except Exception as e:
        logger.warning(f"无法保存缩略图: {e}")
