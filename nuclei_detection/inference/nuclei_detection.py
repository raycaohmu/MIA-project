...
class CellSegmentationInference:
    def __init__(
        ...
        Args:
            ...
            save_all_files (bool, optional): Whether to save all output files (cells.json, cell_detection.json, cells.pt) 
                                      or only geojson files. Defaults to True.
        ...
        self.save_all_files = save_all_files  # 保存参数
    ...
    def process_wsi(
            ...
            batch_size: int = 8,
            geojson: bool = False,
            save_all_files: bool = None, # 是否保存所有文件
    ) -> None:
        """Process WSI file
        ...
        if geojson:
            ...
            # 1. Group cells by patch coordinates
            patch_groups = {}
            for cell in cell_dict_wsi["cells"]:
                patch_id = f"{cell['patch_coordinates'][0]}_{cell['patch_coordinates'][1]}"
                if patch_id not in patch_groups:
                    patch_groups[patch_id] = []
                patch_groups[patch_id].append(cell)

            # 2. Save WSI-level GeoJSON file
            geojson_list = self.convert_geojson(cell_dict_wsi["cells"], True)  # 使用原始格式，保证QuPath兼容性
            with open(outdir / "cells.geojson", "w") as outfile:
                ujson.dump(geojson_list, outfile, indent=2)
            self.logger.info(f"Saved WSI-level GeoJSON to {outdir / 'cells.geojson'}")

            # Pre-create directories
            os.makedirs(outdir / "patch_geojson", exist_ok=True)
            if save_all_files:
                os.makedirs(outdir / "patch_pt", exist_ok=True)

            # Pre-group graph data by patch_id
            self.logger.info("Pre-grouping graph data by patch coordinates")

            # Step 1: Create a dictionary mapping patch_id to indices in cell_dict_wsi
            patch_index_map = {}
            for i, cell in enumerate(cell_dict_wsi["cells"]):
                patch_id = f"{cell['patch_coordinates'][0]}_{cell['patch_coordinates'][1]}"
                if patch_id not in patch_index_map:
                    patch_index_map[patch_id] = []
                patch_index_map[patch_id].append(i)

            # Step 2: Parallelized grouping of graph data
            def build_patch_data(patch_id, indices):
                return patch_id, {
                    "tokens": [graph_data["cell_tokens"][i] for i in indices],
                    "positions": [graph_data["positions"][i] for i in indices],
                    "contours": [graph_data["contours"][i] for i in indices],
            ...
            with ThreadPoolExecutor(max_workers=12) as executor:
                future_to_patch = {
                    executor.submit(build_patch_data, patch_id, indices): patch_id
                    for patch_id, indices in patch_index_map.items()
                }

                for future in future_to_patch:
                    patch_id, patch_data = future.result()
                    patch_data_groups[patch_id] = patch_data

            # 3. Parallel processing of patches
            self.logger.info("Starting parallel processing of patches")

            def process_patch_data(patch_id, patch_cells, patch_data, outdir):
                try:
                    # Save GeoJSON file for the patch using the same format as WSI-level
                    cell_df = pd.DataFrame(patch_cells)
                    patch_geojson_list = []

                    # 遍历每个细胞，为每个细胞创建单独的特征
                    for idx, cell in cell_df.iterrows():
                        contour = cell["contour"]
                        contour.append(contour[0])  # 闭合轮廓
                        
                        cell_geojson_object = get_template_segmentation()
                        cell_geojson_object["id"] = str(uuid.uuid4())
                        cell_geojson_object["geometry"]["coordinates"] = [[contour]]
                        cell_geojson_object["properties"]["classification"]["name"] = TYPE_NUCLEI_DICT[cell["type"]]
                        cell_geojson_object["properties"]["classification"]["color"] = COLOR_DICT[cell["type"]]
                        
                        # 为每个细胞添加其自身的属性
                        cell_geojson_object["properties"]["measurements"] = {
                            "centroid": cell["centroid"],
                            "probability": cell["type_prob"],
                            "patch_coordinates": cell["patch_coordinates"]
                        }
                        
                        patch_geojson_list.append(cell_geojson_object)

                    # 保存patch的GeoJSON文件
                    patch_geojson_path = outdir / "patch_geojson" / f"cells_patch_{patch_id}.geojson"
                    with open(patch_geojson_path, "w") as geojson_file:
                        ujson.dump(patch_geojson_list, geojson_file, indent=2)
                    if save_all_files:
                        # Save PT file for the patch (保持不变)
                        patch_graph = CellGraphDataWSI(
                            x=torch.stack(patch_data["tokens"]),
                            positions=torch.stack(patch_data["positions"]),
                            contours=patch_data["contours"],
                            metadata=patch_data["metadata"],
                        )
                        patch_pt_path = outdir / "patch_pt" / f"cells_patch_{patch_id}.pt"
                        torch.save(patch_graph, patch_pt_path)

                    self.logger.info(f"Successfully processed patch {patch_id}")
                    return patch_id
                except Exception as e:
                    self.logger.error(f"Error processing patch {patch_id}: {str(e)}")
                    return None
            ...
            with ThreadPoolExecutor(max_workers=12) as executor:
                future_to_patch = {
                    executor.submit(
                        ...
                        patch_id,
                        patch_cells,
                        patch_data_groups[patch_id],
                        outdir,
                    ): patch_id
                    for patch_id, patch_cells in patch_groups.items()
                }
                for future in future_to_patch:
                    patch_id = future_to_patch[future]
                    try:
                        result = future.result()
                        if result is None:
                            self.logger.error(f"Patch {patch_id} failed to process")
                    except Exception as e:
                        self.logger.error(f"Unexpected error in patch {patch_id}: {e}")
    ...
    def convert_patch_geojson(
        self, cell_list: List[dict], polygons: bool = False
    ) -> dict:
        """Convert patch cells to a geojson object with detailed cell information

        Args:
            cell_list (list[dict]): Cell list with dict entry for each cell.
            polygons (bool, optional): If polygon segmentations (True) or detection points (False).
        ...
        for idx, cell in cell_df.iterrows():
            if polygons:
                ...
                feature = {
                    ...
                    "properties": {
                        "classification": {
                            "name": TYPE_NUCLEI_DICT[cell["type"]],
                            "color": COLOR_DICT[cell["type"]]
                        },
                        "measurements": {
                            "centroid": cell["centroid"],
                            "probability": cell["type_prob"],
            ...
            geojson_placeholder["features"].append(feature)
...
class CellPostProcessor:
    def __init__(self, cell_list: List[dict], logger: logging.Logger) -> None:
        ...
        # 向量化操作替代 parallel_apply
        self.logger.info("Applying vectorized coordinate conversion")

        # 获取 patch_coordinates 并转换为 NumPy 数组
        coords = np.array(self.cell_df["patch_coordinates"].tolist())

        # 创建 patch_row 和 patch_col 列
        self.cell_df["patch_row"] = coords[:, 0]
        self.cell_df["patch_col"] = coords[:, 1]

        # 转换 patch_coordinates 为字符串格式 "x_y"
        self.cell_df["patch_coordinates"] = [f"{int(row)}_{int(col)}" for row, col in coords]

        self.logger.info("Coordinate conversion completed")
        #print("狗子2")
    ...
    def post_process_cells(self) -> pd.DataFrame:
        ...
        """
        if all(self.cell_df["cell_status"] == 0):
            return self.cell_df
        else:
...
class InferenceWSIParser:
    ...
    def __init__(self) -> None:
        ...
        parser.add_argument(
            "--save_all_files",
            action="store_true",
            help="Set this flag to save all output files. If not set, only geojson files will be saved.",
            default=True,  # 默认为True，保持向后兼容性
        )
...
def check_wsi(wsi: WSI, magnification: float = 40.0):
    ...
    if wsi.metadata["patch_overlap"] not in [0, 64, 128]:
        raise RuntimeError("The patch-overlap must be 0, 64 or 128")
